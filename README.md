...
| **多会话工作流** | 流式回复、历史回放、消息队列、fork、compact、子 agent 会话，以及最多四个会话同屏观察 |
...
- **多会话分屏** —— 把标签拖入对话区或右键选择“加入会话分屏”，最多四个会话同屏。点击蒙版切换唯一输入目标，其他会话继续实时更新；点击分屏外的已有会话或新建会话，都会替换当前选中的一格，不会退出多视图。
