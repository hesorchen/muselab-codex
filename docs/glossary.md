...
**provider** — A vendor configuration record defined in `backend/endpoints.py`. Each provider has a `prefix` (used for longest-prefix routing), a `base_url`, an `env_key` (API key env var name), and flags like `supports_thinking` and `max_output_tokens`. The built-in catalog covers 9 providers, including local-gateway presets; user-created providers use the `c:<slug>` stable ID format. See [`routing.md — Model Resolution`](routing.md#1-model-resolution) and [`providers.md`](providers.md).
